...
var actions = {
    ...
    help: function() {
        ...
        // open modal
        hModal = domUtils.openRegionHelpModal(helpCurrentRegion, helpPreviousRegion, audioData);
        hModal.on('shown.bs.modal', function() {
            ...
            domUtils.appendHelpModalConfig2(hModal, helpCurrentRegion);
...
$(document).ready(function() {
    ...
    $('body').on('change', 'input[name=segment]:radio', function(e) {
        ...
        if (selectedValue === 'previous') {
            domUtils.appendHelpModalConfig2(hModal, helpPreviousRegion);
            helpRegion = {
                start: helpPreviousRegion.start + 0.1,
                end: helpPreviousRegion.end - 0.1
            };
        } else if (selectedValue === 'current') {
            domUtils.appendHelpModalConfig2(hModal, currentRegion);
            helpRegion = {
                start: currentRegion.start + 0.1,
                end: currentRegion.end - 0.1
            };
...
function createRegion(time){
  ...
  //@TODO put this method in javascriptUtils !!
  var hms = javascriptUtils.secondsToHms(toSplit.end);
  $(regionRow).find('.end').text(hms);
  // add a row in the dom
  domUtils.addRegionToDom2(region, javascriptUtils);
/*  var btn = $(regionRow).find('button.fa-trash-o');
  $(btn).addClass(region.id); // @TODO check if still usefull (use only guid)
  $(btn).attr('data-id', region.id); // @TODO check if still usefull (use only guid)
...
function configRegion(elem) {
    ...
    configModal.on('shown.bs.modal', function() {
      var uuid = $('#region-select :selected').val();
      console.log(uuid);
      currentHelpRelatedRegion = getRegionByUuid(uuid);
      console.log(currentHelpRelatedRegion);
    });
...
function onSelectedRegionChange(elem) {
    ...
    var uuid = $('#' + val).find('button.btn-danger').data('uuid');
 ...
 * use html audio player to avoid
 */
...
function addMarker(time, uuid) {
    ...
    dragHandler.title = 'drag';
    dragHandler.dataset.time = time;
    dragHandler.dataset.position = dragHandlerLeft;
    dragHandler.dataset.uuid = uuid || strUtils.createGuid();
    ...
    // if we encounter another drga-handler stop dragging! (ie avoid region overlaps)
    $(".marker-drag-handler").droppable({
...
* therefore we need to store usefull data
*/
function setDragData(time) {
    var result = {};
            ...
            bootbox.confirm(message, function(result) {
                if (result) {
                    ...
                    $('.marker-drag-handler').each(function() {
                      // marker-drag-handler //  .divide-marker
                      var searched = $(this).attr('data-uuid');
                      if(searched == uuid){
